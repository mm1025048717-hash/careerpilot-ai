           ...
           <div className="flex gap-2 overflow-x-auto mt-4 pb-1 scrollbar-hide">
             <button 
               ...
               className={`px-3 py-1.5 rounded-full text-[12px] font-medium transition-all whitespace-nowrap ${
                 !selectedType ? 'bg-[#1D1D1F] text-white' : 'bg-[#F2F2F7] text-[#86868B] hover:bg-[#E5E5EA]'
               }`}
             >
               全部
             </button>
             {Object.keys(DOC_TYPE_NAMES).map(type => (
               <button
                 ...
                 className={`px-3 py-1.5 rounded-full text-[12px] font-medium transition-all whitespace-nowrap ${
                   selectedType === type ? 'bg-[#007AFF] text-white' : 'bg-[#F2F2F7] text-[#86868B] hover:bg-[#E5E5EA]'
                 }`}
               >
                 {DOC_TYPE_NAMES[type]}
               </button>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {loading ? (
            ...
            <div
              ...
              className={`p-3 rounded-[14px] cursor-pointer transition-all border ${
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-[12px] flex items-center justify-center text-[20px] border ${
                  DOC_CONFIG[doc.type]?.bg || 'bg-gray-50 border-gray-100'
                }`}>
                  {DOC_CONFIG[doc.type]?.icon || '📄'}
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className={`text-[14px] font-semibold truncate ${
                    selectedDoc?.id === doc.id ? 'text-[#007AFF]' : 'text-[#1D1D1F]'
                  }`}>
                    {doc.title}
                  </h4>
          ...
          <>
            <header className="px-8 py-5 border-b border-[#F5F5F7] flex justify-between items-center sticky top-0 bg-white/90 backdrop-blur z-10">
               <div className="flex items-center gap-4">
                 <div className={`w-12 h-12 rounded-[16px] flex items-center justify-center text-[24px] shadow-sm border ${
                   DOC_CONFIG[selectedDoc.type]?.bg
                 }`}>
                 ...
                 <div>
                   ...
                   <div className="flex items-center gap-2 mt-1">
                     <span className={`text-[12px] font-medium px-2 py-0.5 rounded-[6px] ${
                       DOC_CONFIG[selectedDoc.type]?.bg.replace('border', 'bg-opacity-50') || 'bg-gray-100 text-gray-600'
